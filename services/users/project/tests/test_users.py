...
class TestUserService(BaseTestCase):
    ...
    def test_add_user(self):
        ...
        add_admin('testa', 'test@test.com', 'test')
        ...
        with self.client:
            ...
            self.assertIn('test@foo.ee was added!', data['message'])
            self.assertIn('success', data['status'])
    ...
    def test_add_user_duplicate_email(self):
        """Ensure error is thwown if the email already exists."""
        add_admin('test', 'test@test.com', 'test')
    ...
    def test_single_user(self):
        ...
        user = add_user('test', 'test@foo.ee', 'greaterthaneight')
        with self.client:
            response = self.client.get(f'/users/{user.id}')
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 200)
            self.assertIn('test', data['data']['username'])
            self.assertIn('test@foo.ee', data['data']['email'])
            self.assertIn('success', data['status'])
    ...
    def test_all_users(self):
        ...
        add_user('test', 'test@foo.ee', 'greaterthaneight')
        add_user('test1', 'test1@foo.ee', 'greaterthaneight')
        with self.client:
            ...
            self.assertIn('test@foo.ee', data['data']['users'][0]['email'])
            ...
            self.assertIn('test1@foo.ee', data['data']['users'][1]['email'])
